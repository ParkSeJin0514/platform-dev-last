...
│         Health Check: OK        │          Health Check: Failover           │
├─────────────────────────────────┼───────────────────────────────────────────┤
│                                 │                                           │
│      AWS ALB (petclinic)        │       GCP Static LB (psj0514-static-lb)   │
│              │                  │                     │                     │
│              ▼                  │                     ▼                     │
│      EKS (petclinic-msa)        │              URL Map                      │
│                                 │          ┌─────────┴─────────┐            │
│                                 │          ▼                   ▼            │
│                                 │   petclinic-gke-backend   GCS Bucket      │
│                                 │   (GKE 정상 시)           (GKE 장애 시)   │
│                                 │          │                   │            │
│                                 │          ▼                   ▼            │
│                                 │   petclinic-api-gateway-neg  index.html   │
│                                 │   (Standalone NEG)       (정적 fallback) │
│                                 │          │                                │
│                                 │          ▼                                │
│                                 │   GKE Pod (api-gateway:8080)              │
└─────────────────────────────────┴───────────────────────────────────────────┘
```

### GCP Load Balancer 구성

GCP Secondary 환경에서는 **Static External HTTP(S) Load Balancer**를 사용합니다:

| 구성 요소 | 이름 | 역할 |
|-----------|------|------|
| Forwarding Rule | `psj0514-static-lb-forwarding-rule` | 외부 트래픽 수신 |
| Target HTTP Proxy | `psj0514-static-lb-target-proxy` | HTTP 요청 처리 |
| URL Map | `psj0514-static-lb` | 백엔드 라우팅 결정 |
| Backend Service | `petclinic-gke-backend` | GKE로 트래픽 전달 |
| Backend Bucket | (GCS) | GKE 장애 시 fallback |
| NEG | `petclinic-api-gateway-neg` | GKE Pod 엔드포인트 |

### Standalone NEG 설정

GKE 재배포 시에도 NEG 이름이 변경되지 않도록 **Standalone NEG**를 사용합니다.

**문제점**: 기본 GKE NEG는 자동 생성된 이름(k8s1-xxx-xxx)을 사용하여, GKE 삭제/재생성 시 이름이 변경됩니다. 이로 인해 Static LB의 Backend Service 설정을 수동으로 업데이트해야 합니다.

**해결책**: `cloud.google.com/neg` 어노테이션으로 NEG 이름을 고정합니다.

```yaml
# petclinic-gitops/overlays/gcp/service-patch.yaml
apiVersion: v1
kind: Service
metadata:
  name: api-gateway
...
**작동 방식**:
1. GKE Service 배포 시 `petclinic-api-gateway-neg` 이름으로 NEG 자동 생성
2. Static LB의 `petclinic-gke-backend`가 해당 NEG 참조
3. GKE 삭제/재생성 후에도 동일한 이름의 NEG가 생성됨
4. Backend Service 설정 변경 불필요

### DR Failover 시나리오

| 상태 | AWS | GCP GKE | Route 53 | 사용자 접속 |
|------|-----|---------|----------|------------|
| 정상 | ✅ Primary | ✅ Standby | → AWS ALB | petclinic (AWS) |
| AWS 장애 | ❌ Down | ✅ Active | → GCP LB | petclinic (GCP) |
| AWS+GKE 장애 | ❌ Down | ❌ Down | → GCP LB | index.html (GCS) |

### NEG 삭제 시 주의사항

Terraform Destroy 시 NEG는 Backend Service에 연결되어 있어 직접 삭제할 수 없습니다.

**삭제 순서** (terraform-destroy.yml에서 자동 처리):
1. Backend Service 삭제 (`petclinic-gke-backend` 등)
2. NEG 삭제 (`petclinic-api-gateway-neg`, `k8s1-*`, `k8s2-*`)
3. VPC 삭제

> **중요**: DR용 리소스(`psj0514-*`)는 terraform-destroy.yml에서 보호되어 삭제되지 않습니다. 전체 삭제가 필요한 경우 수동으로 삭제해야 합니다.
